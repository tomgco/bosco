...
module.exports = function(bosco) {
	...
	function getStaticAssets(repos, minified, tagFilter, cdnUrl, next) {
		...
		async.mapSeries(repos, loadService, function(err, services) {
			...
			async.mapSeries(services, function(service,next) {				
				createAssetList(service, minified, tagFilter, cdnUrl, next);
				...
				if(minified) {
					getLastCommitForAssets(staticAssets, function(err, staticAssets) {	
						minify(staticAssets, cdnUrl, function(err, staticAssets) {
	...
	function createAssetList(boscoRepo, minified, tagFilter, cdnUrl, next) {
	...
	function minify(staticAssets, cdnUrl, next) {
		...
		async.parallel([
			function pcompileJs(next) {
				compileJs(jsAssets, cdnUrl, next);
			},
	...
	function compileJs(jsAssets, cdnUrl, next) {

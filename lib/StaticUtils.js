...
module.exports = function(bosco) {
	...
	function doBuild(build, watchBuilds, reloadOnly, tagFilter, next) {
		...
		if(watchBuilds) {
			...
			var wc = spawn(command, cmdArray, {cwd:build.repoPath});
			var finishedText = build.watch.finished || 'finished';
			var stdout = "", calledReady = false;
			...
			var checkFinished = function() {
				if(stdout.indexOf(finishedText) >= 0 && !calledReady) {
					calledReady = true;
					setTimeout(function() {
						buildFinished(null, stdout.toString(), null);	
					}, checkDelay);
				} else {
			...
			exec(command, {cwd:build.repoPath}, buildFinished);

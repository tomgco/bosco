...
module.exports = function(bosco) {
	...
	function getStaticAssets(repos, minified, tagFilter, reloadOnly, next) {
		...
		async.mapSeries(repos, loadService, function(err, services) {
			...
			async.mapSeries(services, function(service, cb) {				
				createAssetList(service, minified, tagFilter, cb);
				...
				async.mapSeries(builds, function(build, cb) {
					...
					doBuild(build, reloadOnly, tagFilter, cb);
	...
	function doBuild(build, reloadOnly, tagFilter, next) {

		var command = reloadOnly ? "echo 'Not running build as change triggered by external build tool'" : build.command;
		bosco.log("Running build command for " + build.name.blue + ": " + build.command);
		exec(command, {cwd:build.repoPath}, function(err, stdout, stderr) {
			if(err) {
				bosco.error(stderr);
			}
	...
	function getAssetHelper(bosco, boscoRepo, tagFilter) {
		...
		return {
			addAsset: function(staticAssets, assetKey, asset, tag, type) {					
				...
				var checkDuplicate = function(newAsset) {
					var duplicate = false;
	...
	function createAssetList(boscoRepo, minified, tagFilter, next) {
			...
			var assetKey,staticAssets = {}, staticBuild = {},
	...
	function createKey(tag, hash, type, extension) {
		return bosco.options.environment 
				+ '/' + (bosco.options.build ? bosco.options.build + '/' : '') 
				+ type 
				+ '/' + tag 
				+ (hash ? '.' + hash : '') 
				+ (extension ? '.' + extension : '');
